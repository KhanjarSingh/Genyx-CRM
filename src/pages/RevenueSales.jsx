...
export function RevenueSales() {
  ...
  const roiEngineSection = (
    <section className="grid grid-cols-1 lg:grid-cols-12 gap-6">
      <motion.div drag dragConstraints={{ left: 0, right: 0, top: 0, bottom: 0 }} dragElastic={0.05} className="lg:col-span-5 cursor-grab active:cursor-grabbing">
      ...
      <motion.div drag dragConstraints={{ left: 0, right: 0, top: 0, bottom: 0 }} dragElastic={0.05} className="lg:col-span-7 cursor-grab active:cursor-grabbing">
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 h-full">
          {revenueHealthKPIs.map((kpi, i) => (
            <Card key={i} className="hover:shadow-md hover:border-brand/30 transition-all h-full p-6">
              <div className="flex items-center gap-2 mb-4">
                <div className={`w-8 h-8 rounded-lg ${KPI_CONFIGS[kpi.color]?.iconBg || 'bg-gray-100 dark:bg-gray-800'} flex items-center justify-center`}>
                  <DollarSign className={`w-4 h-4 ${KPI_CONFIGS[kpi.color]?.iconColor || 'text-gray-400 dark:text-gray-500'}`} />
                </div>
